...
contract DaoVotePlatformTest is Test {
    ...
    function test_updateUserWeight() public {
        ...
        assertEq(dao.getYes(pid), yesBefore);
        assertEq(dao.getNo(pid), noBefore);

        dao.forceUpdateDelegate(bob);

        assertTrue(dao.getYes(pid) > yesBefore || dao.getNo(pid) > noBefore);
    }
    ...
    function test_updateUserWeightCannotCallAfterVoting() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DaoVotePlatform.AlreadyVoted.selector);
        dao.updateUserWeight(alice);
    }
    ...
    function test_updateUserWeightCannotCallTwice() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DaoVotePlatform.AlreadyUpdated.selector);
        dao.updateUserWeight(alice);
    }
    ...
    // ========== Weight change with pending ==========

    function test_updateUserWeightUpdatesDelegateYesTotal() public {
        _lockAndDelegate(alice, 500, bob);
        _lockAndDelegate(bob, 2000, address(0));
        _warpToNextEpoch();
        ...
        assertTrue(dao.getYes(pid) + dao.getNo(pid) > yesBefore + noBefore);
    }

    function test_forceUpdateDelegate() public {
        _lockAndDelegate(alice, 500, bob);
        _lockAndDelegate(bob, 2000, address(0));
        _warpToNextEpoch();
        ...
        (,,,, int256 adj) = dao.getVote(pid, bob);
        assertTrue(adj > 0);
    }
